...
- `crates/vernier-mask/` — pure Rust COCO RLE codec, polygon rasterizer, and mask ops (per ADR-0009). Leaf crate: depended on by `vernier-core`, never the reverse. **No Python deps.** `#![forbid(unsafe_code)]`. Mirrors `pycocotools.mask` but with a `MaskError` enum instead of `0`/`-1`/empty-RLE sentinels.
