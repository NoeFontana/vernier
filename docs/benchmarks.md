...
## Threading scaling (preview)

[ADR-0047](https://github.com/NoeFontana/vernier/blob/main/docs/adr/0047-threading-model.md)
adds an opt-in `num_threads` kwarg to `vernier.instance.Evaluator.evaluate`
and a matching axis to the bench harness. The plumbing — workload
classes, `CellSpec`, runner argv, CLI fan-out — is validated by the
`synthetic_threads_smoke` workload (100 images, 10 categories,
`num_threads ∈ {1, 2, 4, 8}`); the full scaling sweep over val2017,
LVIS, panoptic, and ADE20K is deferred to a separate operation and
will replace this preview with reproducible numbers + a regenerated
table.

Regenerate the smoke numbers below with `just bench-threads-smoke`.
The result tree lands under
`bench/results/<sha>/<fp>/instance/synthetic_threads_smoke_t{N}/bbox/vernier.json`
— one cell per pinned thread count.

| num_threads | median wall (ns) | source |
| ----------- | ----------------:| ------ |
| 1 | _regenerated by `just bench-threads-smoke`_ | smoke |
| 2 | _regenerated by `just bench-threads-smoke`_ | smoke |
| 4 | _regenerated by `just bench-threads-smoke`_ | smoke |
| 8 | _regenerated by `just bench-threads-smoke`_ | smoke |
