...
| `parity_mode` | string, one of `strict` / `corrected`               | The kernel-resolved parity mode. `--parity-mode aligned` is a retired alias that collapses to `strict` (ADR-0002's 2026-05-10 amendment folded the tier in), so the JSON only ever carries the two kernel modes. Default `strict` (ADR-0015).                                                                                              |
