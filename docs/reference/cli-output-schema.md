...
| `parity_mode` | string, one of `strict` / `corrected`               | The kernel-resolved parity mode — the same two values `--parity-mode` accepts (ADR-0002). Default `strict` (ADR-0015). ADR-0059 removed the third value the flag once took; `--parity-mode aligned` is now an argument error, so no invocation can produce a third string here.                                                                                              |
