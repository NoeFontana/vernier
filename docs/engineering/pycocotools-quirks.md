...
| I7 | `bbIou` computes the union as `u = da+ga-i` in C. GCC's default for C is `-ffp-contract=fast`, and on aarch64 `fmadd` is baseline — so on that architecture the reference is, in principle, free to fuse a multiply into the union and drop a rounding step, which would move the final IoU by 1–2 ULP on roughly one COCO-scale box pair in 10^6. The published wheels do **not**: `bbIou` is an exported symbol, and disassembly of pycocotools 2.0.11 `cp312-abi3-manylinux_2_17_aarch64`, `cp39-manylinux_2_17_aarch64` and `cp312-abi3-musllinux_1_2_aarch64` shows separate `fmul`/`fadd`/`fsub`/`fdiv` and no `fmadd`/`fmsub`/`fnmsub`/`fmla` (verified 2026-09-17). It is structural, not luck: `i` feeds both `u` and the numerator `i/u`, and `da` feeds both `da+ga` and the crowd branch (**E1**), so fusing either would force the compiler to keep the rounded value *and* recompute the product. | mc:133 | **strict**. vernier pins the non-contracting (x86-64) form on *every* architecture — `expected_bits` is one table, asserted on both shipped arches — so vernier's own output is architecture-independent and bit-equal to the published wheel on both. The two-tier vocabulary has no per-arch cell and this row needs none: the residual hazard is a pycocotools **rebuilt from the sdist** by the user's own compiler (`pip install --no-binary`, distro, conda-forge), not the pinned oracle. Against such a build on aarch64, vernier may differ by 1–2 ULP — a divergence introduced by that toolchain. Ratified by [ADR-0056](../adr/0056-pin-no-fp-contraction-for-bbox-iou.md). | `similarity/bbox.rs::fp_contract_pin` (golden `expected_bits` table + `scalar` / dispatched / self-validation tests); CI job `test-rust-cross-arch` runs it `--release` on `ubuntu-latest` and `ubuntu-24.04-arm`. ✓ |
