...
| J3 | DT `area` is auto-derived by `loadRes`: from `bbox` for bbox results; from `maskUtils.area(seg)` for segm; from the keypoint-bbox `(x1-x0)*(y1-y0)` for keypoints. The keypoint area is *not* the GT person area; it can be tiny if the visible keypoints are clustered, dropping a DT into the "small" bucket unexpectedly. | co:342, co:349, co:361 | **strict** (drives small/medium/large bucketing — changing breaks scores). The JSON and array paths derive from `bbox` by default; `dt_area="mask"` on the segm / boundary grids reproduces `loadRes`'s segm branch, `maskUtils.area` of the detection's RLE (compressed, uncompressed or bitmask; an error when the detection has no RLE). | `dataset.rs::j3_derives_area_from_bbox`, `j3_mask_area_is_the_rle_foreground_for_both_count_forms`, `j3_mask_area_rejects_detections_without_an_rle`; `tests/python/test_dt_area_mask.py`. ✓ |
