...
| # | Quirk | Source | Disposition | Wired-by |
|---|---|---|---|---|
| J1 | DT `id` is overwritten with sequential 1..N regardless of any `id` field in the input. So `evalImgs['dtIds']` does not match user input. Round-tripping requires a separate user→assigned mapping. | co:333, co:343, co:352, co:362 | **strict**. Vernier preserves user ids when present, auto-assigns 1..N otherwise; pycocotools-shaped outputs are bit-equal. | `dataset.rs::j1_auto_assigns_ids_when_absent:860`, `j1_preserves_user_supplied_ids:871`; the list and `(N, 7)` matrix routes (ADR-0057) inherit this — `tests/python/test_ingest_route_equivalence.py::test_detection_ids_are_assigned_by_input_position`, `::test_supplied_ids_are_preserved_on_the_list_route`. ✓ |
| J2 | Bbox-only DT: a fake rectangular "polygon" segmentation `[[x1,y1, x1,y2, x2,y2, x2,y1]]` is synthesized. If the user later switches `iouType='segm'`, those DTs are evaluated against this rasterized rectangle, not the user's intended box. | co:341 | **strict** (silent path-dependence). Document loudly. | `evaluate.rs::synthesize_dt_segm_from_bbox:770` (strict) / `missing_segmentation_err` (corrected, raises naming the DT and image). All three ADR-0057 ingest routes inherit it — a bbox-only payload as bytes, as result dicts and as an `(N, 7)` matrix evaluates identically: `tests/python/test_ingest_route_equivalence.py::test_bbox_only_under_segm_is_j2_on_every_route`, `::test_bbox_only_under_segm_is_refused_in_corrected_mode`. ✓ |
...
| J4 | DT `iscrowd = 0` always. Per E2. | co:344, co:353 | **strict**. | `dataset.rs:469-484`; test `loads_detections_from_json_array:917`; ADR-0057 routes covered by `tests/python/test_ingest_route_equivalence.py::test_area_and_iscrowd_fields_are_ignored`. ✓ |
...
- **A handful of corrected:** A3 (flat-index re-accumulate), C3 (bare except), D1 (overwritten ignore field), H2 (silent merge), I2 (-1 sentinel), I6 (1-D reshape bug), J6 (heterogeneous DT dispatch — corrected for the *derivation*; route selection still reads entry 0, see ADR-0057), K1 (2-point polygon collision), L3/L5/L6/L7 (API hygiene), F1 (per-category sigmas).
