...
| B5 | `dtm` and `gtm` store the *opposite* side's id (DT row stores matched GT id; GT row stores matched DT id). Stored as float64 (zero-init array). IDs become floats and must be cast back. | ce:295-296, ce:268-269 | **strict**. We store as int with a sentinel and cast at the boundary; bit-equal observable output. | `matching.rs:83-87` (`Array2<i64>` -1 sentinel); `harness.py:146` (snapshot float64 coercion). ✓ |
...
| # | Quirk | Source | Disposition | Wired-by |
|---|---|---|---|---|
| C1 | 101-point recall integration: `recThrs = linspace(0, 1, 101)`. AP is the mean of interpolated precision at exactly these 101 recalls. Uses `np.searchsorted(rc, recThrs, side='left')` to find the first cumulative recall `>= rt`. | ce:507, ce:402 | **strict**. Trapezoid integration would be measurably different on noisy curves. | `accumulate.rs:378`; test `partition_point_matches_numpy_searchsorted_left:559`. ✓ |
| C2 | Precision envelope: descending sweep `for i in range(nd-1, 0, -1): if pr[i] > pr[i-1]: pr[i-1] = pr[i]`. Standard PASCAL "interpolated AP" — precision at recall ≥ r is the *max* precision achievable at any recall ≥ r. | ce:398-400 | **strict**. | `accumulate.rs:357-362`; test `precision_envelope_runs_right_to_left:536`. ✓ |
| C3 | If a recall threshold is unreachable (`searchsorted` returns `nd`), the bare `try/except: pass` swallows the resulting IndexError and leaves remaining `q[ri]`/`ss[ri]` at zero. Means precision at unreached recalls is exactly **0**, not -1. | ce:402-408 | **strict** but **rewrite without `except:`**. We can do `inds = clip(inds, 0, nd-1)` plus an explicit mask of unreached rows; same numerical output, no swallowed exceptions. | `accumulate.rs:364-386`; test `lone_fp_yields_zero_recall_zero_precision:491`. ✓ |
| C4 | `recall[t,k,a,m] = rc[-1] if nd else 0` — "AR" is the **terminal** cumulative recall (i.e., total TPs / total positives), *not* the area under a PR curve's recall axis. Many users expect "AR@N" to be an integral; it's not. | ce:389-392 | **strict**. Document loudly. | `accumulate.rs:355`; test `perfect_match_yields_ap_one_and_ar_one:474`. ✓ |
| C5 | Absent-category sentinel: `precision`, `recall`, `scores` arrays initialized to `-1`. `summarize()` filters with `s[s>-1]` before computing the mean. So a category with no GTs disappears from the average; it does not get counted as zero. The `-1` is preserved **only** for cells skipped before the per-t `q = np.zeros((R,))` assignment site — i.e. `len(E) == 0` or `npig == 0` (the two `continue`s). Cells with `npig > 0` but no detections (`n_d == 0`) still reach the assignment and land at `0.0`, because the inner `try/except` over an empty `pr` bails on the first index and `q` is written verbatim (this overlap with quirk **C3** is by design). | ce:334-336, ce:452-455 | **strict**. | `accumulate.rs:222-224` (sentinel init) + `309-314` (`n_d==0` zero-fill), `summarize.rs:723` (`v > -1.0` filter); tests `cell_with_only_ignore_gts_skips_entirely:464`, `no_dt_with_real_gt_yields_zero_recall_and_zero_precision:438`. ✓ |
| C6 | `pr.tolist()` and `q.tolist()` then iterate as Python lists for the inner monotonic sweep — explicit comment "numpy is slow without cython optimization for accessing elements". A vector op (`np.maximum.accumulate(pr[::-1])[::-1]`) gives the same result much faster. | ce:396 | **strict**. Vernier needs no `.tolist()`: the sweep is a scalar right-to-left loop directly over `Vec<f64>` — the *same* operation as **C2**'s precision envelope — so it is trivially bit-equal to pycocotools' list-iterated form. | `accumulate.rs:357-362` (shared with **C2**); covered by `precision_envelope_runs_right_to_left:536`. ✓ |
| C7 | TP/FP are computed as `logical_and(dtm, logical_not(dtIg))` and `logical_and(logical_not(dtm), logical_not(dtIg))`. A DT that matched an ignore-GT is in *neither* set. | ce:375-376 | **strict**. | `accumulate.rs:339-352`; test `ignored_dt_does_not_count_as_fp:513`. ✓ |
| C8 | `np.spacing(1)` ≈ 2.22e-16 used as eps in `pr = tp / (fp+tp+np.spacing(1))`. Per-platform/numpy-version reproducible but *not* a clean constant. | ce:385 | **strict**. eps pinned to `f64::EPSILON`, bit-equal to `np.spacing(1)` on every supported platform. | `parity.rs::PARITY_EPS:42`; test `parity_eps_matches_numpy_spacing_1:109`. ✓ |
...
| D3 | `_ignore` is a per-evaluation flag: `True` if `gt['ignore']` is set OR `gt['area']` is outside the current area range. So changing `aRng` rewrites `_ignore` mid-loop. The same GT object is mutated in place. | ce:250-254 | **strict**. Vernier computes `_ignore` per-call without mutating; outputs bit-equal. | `evaluate.rs:550-556` (no-mutation by `Arc<Vec<…>>` construction). ✓ |
| D4 | Default `areaRng` for det: `[[0, 1e10], [0, 1024], [1024, 9216], [9216, 1e10]]`. The "all" bucket is `[0, 1e10]` — floats; `1e5**2` is exactly `1e10`. | ce:509 | **strict** (these are user-visible defaults; changing them breaks every published score). | `evaluate.rs::AreaRange::coco_default:170`; literal-pinning test `d4_coco_default_area_ranges_pin_literal_values:1926`. ✓ |
...
| D6 | Area-range filter on GT uses **strict** inequalities: `g['area'] < aRng[0] or g['area'] > aRng[1]`. So a GT with area exactly `1024` falls in *both* small (`[0, 1024]`) and medium (`[1024, 9216]`). Buckets are not partitions. | ce:251 | **strict**. | `evaluate.rs::AreaRange::contains:221-226` (`area >= lo && area <= hi` — the De Morgan of pycocotools' `not (area < lo or area > hi)`); wired into the GT `_ignore` filter (`evaluate.rs:1755`) and DT area test (`evaluate.rs:1793`); fixture `boundary_area_segm` (in `ALL_FIXTURES`). ✓ |
...
| E3 | The `iou()` API takes `iscrowd` as a length-`n_gt` array. There is no DT-side iscrowd vector. The Cython assertion `crowd_length == n` enforces gt-length only. | mp:218-219 | **strict**. | `similarity/mod.rs::Similarity::compute:49-54` (type signature has no DT-side iscrowd vector); the gt-length assertion is enforced + tested by `matching.rs::gt_iscrowd_length_mismatch_is_typed_error:467`. ✓ |
...
| F2 | OKS area uses `gt['area'] + np.spacing(1)`. The `+spacing` is purely a divide-by-zero guard. | ce:229 | **strict**. eps pinned to `f64::EPSILON` per C8; bit-equal output. | Ratified by [ADR-0012](../adr/0012-oks-keypoints-surface.md). Implemented by `OksSimilarity::compute` (`area + f64::EPSILON`) at `crates/vernier-core/src/similarity/oks.rs`. |
...
| G3 | Sign extension on decode: `if(!more && (c & 0x10)) x \|= -1 << 5*k`. Implementation-defined behavior in C (left-shift of negative). Needs an explicit two's-complement formulation in Rust. | mc:261 | **strict**. Rust port computes `x \|= -(1i64 << (5*k))` (defined two's-complement); decoded counts bit-equal. | `codec.rs::negative_differential_uses_sign_extension:148`. ✓ |
...
| I3 | `rleIou` `i==0 → u=1` to avoid 0/0; `bbIou` short-circuits when `w<=0` or `h<=0` (no division at all). Asymmetric guards. | mc:109, mc:131-132 | **strict**. Both cases yield IoU=0; vernier uses a single clean guard, outputs bit-equal. | `similarity/bbox.rs::zero_area_gt_with_zero_inter_yields_zero_not_nan:247`, `:258`; `segm.rs::empty_gt_or_dt_pair_is_zero_not_nan:218`. ✓ |
...
| I5 | `iou(dts, gts, iscrowd)` Cython entry returns Python `[]` when either side is empty. Not a 0-shape numpy array. | mp:220-221 | **strict**. Empty matrix surfaces as a 0-shape ndarray at the Python boundary; downstream behavior bit-equal. | `similarity/bbox.rs::empty_inputs_return_unchanged_matrix:285`, `segm.rs:258`. ✓ |
...
| L3 | `Params.useSegm` deprecated but still honored: if set, overrides `iouType`. Prints a warning. | ce:130-132, ce:534 | **corrected**. Drop in vernier; useSegm has been deprecated for years. | `python/vernier/_compat.py::_reject_use_segm:385` (called from `setDetParams` at `:330`); tests `test_default_use_segm_is_none_and_does_not_raise:476`, `test_assigned_use_segm_raises_with_l3_message:489`. ✓ |
| L4 | `Params.useCats` is `1` (int) not `True` (bool). Comparisons via truthiness throughout. | ce:511, ce:328 | **strict**. Vernier accepts int-or-bool at the boundary; matching behavior bit-equal. | `dataset.rs::indices_for_image:912`; `tests/python/test_compat.py:151`. ✓ |
