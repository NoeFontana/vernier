...
| A2 | `p.maxDets = sorted(p.maxDets)` — user input order is silently overwritten. Affects which `AR@1/10/100` slot maps to which threshold. | ce:137 | **strict**. Same sort; outputs bit-equal. | *pending: PR `phase2/audit-a2-sort-maxdets`* |
