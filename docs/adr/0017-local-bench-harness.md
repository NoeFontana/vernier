...
Predictions generated by *jittering GT*: deterministic Gaussian noise
on bbox coords / mask boundaries / keypoint positions, with a
controlled fraction of false positives, false negatives, and
confidence drawn from a beta biased toward correct-detection. The
seed is part of the workload identity (`workload_id` includes it),
which makes a workload reproducible across machines. Three reasons
to prefer this over real-model predictions for v1:
...
3. The cocoapi unit-test fixtures already use this pattern, so
   it's not new methodology — we're following the precedent of
   the oracle we benchmark against.
...
**Reference predictions** (deferred to v2). A real model's outputs
from git-lfs or a pinned URL. Adds external dependency and a model-
license question; the jittered-GT workload is sufficient for v1's
goals (latency comparison + parity coupling). Out of scope here.
...
- **Reference-model predictions.** Real-model outputs as a workload
  family. Adds dependency and license complexity; the jittered-GT
  workload is sufficient for v1 goals.
