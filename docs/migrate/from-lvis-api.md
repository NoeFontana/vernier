...
> **Status.** LVIS federated semantics match `lvis-api` bit-equal on
> the `(T, R, K, A)` precision tensor at full LVIS v1 val under
> `ParityMode::Strict` (the default for users migrating from `lvis-api`).
